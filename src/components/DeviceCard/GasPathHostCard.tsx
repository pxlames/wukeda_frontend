...
import { GasPathHostDevice } from '../../types/device.types';
...
interface GasPathHostCardProps {
  device: GasPathHostDevice;
  headerImage: string;
  bottomImage: string;
}

export const GasPathHostCard = ({
  device,
  headerImage,
  bottomImage,
}: GasPathHostCardProps): JSX.Element => {
  const { parameters, status, interfaceName, online } = device;

  // 从parameters中提取参数（根据实际后端返回的字段调整）
  const details = [
  ...
  const getStatusColor = (statusValue?: string) => {
    if (statusValue === '正常运行' || statusValue === '运行') return 'text-[#3cda54]';
    if (statusValue === '故障') return 'text-[#ff4444]';
    if (statusValue === '停止' || statusValue === '停机') return 'text-[#ffffffcc]';
    return 'text-[#ffffffcc]';
  };

  // 获取状态文本
  const getStatusText = () => {
    return status.operating || status.device_on_off || status.running || '未知';
  };
    ...
    <article className="relative w-[570px] h-[407px] overflow-hidden">
      ...
      <div className="absolute top-0 left-0 w-[570px] h-[407px]">
        ...
        <div className="absolute top-16 left-[42px] w-[493px] h-[25px] flex justify-between items-center">
          ...
          <div className={`flex items-center justify-center [font-family:'Source_Han_Sans_SC-Medium',Helvetica] font-bold text-base tracking-[0] leading-[normal] whitespace-nowrap ${getStatusColor(getStatusText())}`}>
            {getStatusText()}
          </div>
        </div>
